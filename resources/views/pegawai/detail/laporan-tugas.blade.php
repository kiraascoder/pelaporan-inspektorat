...
@section('content')
    <div class="space-y-6">
        ...
        @php
            $auth = auth()->user();
            $authUserId = $auth->user_id ?? $auth->id;

            $isKetua = false;

            if ($tim) {
                $anggotaLogin = $tim->anggotaAktif->first(function ($anggota) use ($authUserId) {
                    $anggotaId = $anggota->user_id ?? $anggota->id;
                    return (string) $anggotaId === (string) $authUserId;
                });

                $isKetua = $anggotaLogin && ($anggotaLogin->pivot->role_dalam_tim ?? null) === 'Ketua';
            }
                ...
                <div class="pt-4 border-t">
                    ...
                    <div class="flex flex-wrap gap-2">
                        @foreach ($lampiran as $i => $file)
                            @php
                                $path = is_array($file)
                                    ? $file['path_file'] ?? ($file['path'] ?? ($file['file'] ?? null))
                                    : $file;

                                $namaFile = is_array($file)
                                    ? $file['nama_file'] ?? basename($path ?? 'Lampiran')
                                    : basename($path ?? 'Lampiran');
                            @endphp

                            @if ($path)
                                <a href="{{ asset('storage/' . $path) }}" target="_blank"
                                    class="inline-flex items-center px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs hover:bg-blue-100">
                                    📎 Lampiran {{ $i + 1 }}
                                    <span class="ml-1 text-blue-500">({{ $namaFile }})</span>
                                </a>
                            @endif
                        @endforeach
                    </div>
            ...
            <div class="flex justify-between items-center mb-4">
                <div>
                    ...
                    <p class="text-sm text-gray-500 mt-1">
                        Laporan tugas dibuat oleh ketua tim dan akan disetujui oleh ketua bidang.
                    </p>
                </div>

                <span class="text-sm text-gray-500">
                    Total: {{ $laporanTugas->count() }}
                </span>
            </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full border text-sm">
                        ...
                        <tbody>
                            ...
                            @foreach ($laporanTugas as $lt)
                                {{-- Anggota biasa tidak melihat laporan yang masih Draft --}}
                                ...
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2 border text-center">
                                        <span
                                            class="inline-block px-3 py-1 rounded-full text-xs font-medium {{ $statusLaporanTugasColors[$lt->status_laporan] ?? 'bg-gray-100 text-gray-800' }}">
                                            {{ $lt->status_laporan }}
                                        </span>
                                    </td>
                                    ...
                                    <td class="px-4 py-2 border text-center">
                                        <div class="flex items-center justify-center gap-3">
                                            ...
                                            {{-- Ketua tim hanya bisa edit saat Draft --}}
                                            @if ($isKetua && $lt->status_laporan === 'Draft')
                                            ...
                                            {{-- Setelah submit, tunggu Ketua Bidang --}}
                                            @if ($lt->status_laporan === 'Submitted')
                                                <span class="text-xs text-yellow-700 bg-yellow-50 px-2 py-1 rounded-full">
                                                    Menunggu approval Ketua Bidang
                                                </span>
                                            @elseif ($lt->status_laporan === 'Reviewed')
                                                <span class="text-xs text-blue-700 bg-blue-50 px-2 py-1 rounded-full">
                                                    Sedang direview Ketua Bidang
                                                </span>
                                            @elseif ($lt->status_laporan === 'Approved')
                                                <span class="text-xs text-green-700 bg-green-50 px-2 py-1 rounded-full">
                                                    Disetujui Ketua Bidang
                                                </span>
                                            @elseif ($lt->status_laporan === 'Rejected')
                                                <span class="text-xs text-red-700 bg-red-50 px-2 py-1 rounded-full">
                                                    Ditolak Ketua Bidang
                                                </span>
                                            @endif

                                            {{-- Download hanya tampil setelah laporan dikirim --}}
                                            ...
                                            {{-- Jika Draft dan bukan ketua, tidak ada aksi --}}
                                            @if (!$isKetua && $lt->status_laporan === 'Draft')
                            ...
                            @if ($nomor === 1)
                                <tr>
                                    <td colspan="6" class="px-4 py-8 border text-center text-sm text-gray-500">
                                        Belum ada laporan tugas yang dapat ditampilkan.
                                    </td>
                                </tr>
                            @endif
                        </tbody>
                        ...
                        <div class="px-6 py-4 space-y-6">
                                ...
                                <div class="grid sm:grid-cols-2 gap-2" id="edit_temuan_pemeriksaan_container">
                                    @foreach ($opsi as $label)
                                        <label
                                            ...
                                            <input type="checkbox" name="temuan_pemeriksaan[]"
                                                value="{{ $label }}"
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">
                                    Format: PDF, DOC, JPG, PNG. Max 5MB/file
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <p class="text-xs text-gray-500 mt-1">
                                    Jika memilih Submitted, laporan akan dikirim ke Ketua Bidang untuk approval.
                                </p>
                            </div>
                        </div>
